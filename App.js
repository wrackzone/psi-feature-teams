...
Ext.define('CustomApp', {
    ...
    launch: function() {
        ...
        configs.push({ model : "TypeDefinition",
                       fetch : true,
                       filters : [ { property:"Name", operator:"=", value:"Hierarchical Requirement"} ]
        });

        // get the valid values for accepted
        Rally.data.ModelFactory.getModel({
            type : "HierarchicalRequirement",
            success: function(userStoryModel) {
                userStoryModel.getField("ScheduleState").getAllowedValueStore().load(
                    {
                        callback : function(records,op,success) {
                            var values = _.map(records,function(r) { return r.get("StringValue");});
                            var accIndex = _.indexOf(values, "Accepted");
                            app.acceptedValues = accIndex < values.length ?
                                _.last(values,2) : _.last(values,1)
                            console.log("Accepted Values:",app.acceptedValues);
                            app.runQueries(configs);
                        }
                    }
                );
            }
    ...
    runQueries : function(configs) {

        async.map( configs, this.wsapiQuery, function(err,results) {
            app.peRecords = results[0];
            app.projects  = results[1];
            app.releases  = results[2];
            app.iterations = results[3];
            app.featureType = results[4][0].get("TypePath");
            app.createReleaseCombo(app.releases);
        });
    ...
    //                     var accepted = _.reduce( stories, function(memo,child) {return memo + ( _.indexOf(app.acceptedValues, child.get("ScheduleState")) !== -1 ? child.get("PlanEstimate") :0);},0);
            ...
            listeners: {
                ...
                load: function(store, data, success) {
                    ...
                    _.each( _.keys(grouped), function(key) {
                        ...
                        var total    = _.reduce( stories, function(memo,child) {return memo + child.get("PlanEstimate");},0);
                        // var accepted = _.reduce( stories, function(memo,child) {return memo + ( child.get("ScheduleState")=="Accepted" ? child.get("PlanEstimate") :0);},0);
                        var accepted = _.reduce( stories, function(memo,child) {return memo + ( 
                            _.indexOf( app.acceptedValues, child.get("ScheduleState"))!==-1 ? child.get("PlanEstimate") :0);},0);
                        var p        = total > 0 ? (accepted/total) * 100 : 0;
                        row.Teams = _.isUndefined(row.Teams) ? {} : row.Teams;
                        // row["Teams"][key] = p;
                        row.Teams[key] = {progress:p,total:total,accepted:accepted};
